...
class ToxicityDetector:
    ...
    async def predict(self, text: str) -> ToxicityScores:
        ...
        try:
            ...
            results = await loop.run_in_executor(None, self._predict_sync, text)
    ...
    async def predict_batch(self, texts: list[str], batch_size: int = 32) -> list[ToxicityScores]:
        ...
        try:
            ...
            for idx, batch_result in zip(non_empty_indices, batch_results, strict=True):
    ...
    def _predict_batch_sync(self, texts: list[str], batch_size: int) -> list[dict[str, float]]:
        ...
        for i in range(0, len(texts), batch_size):
            ...
            for j in range(num_samples):
                sample_result = {
                    key: float(value[j].item() if hasattr(value[j], "item") else value[j])
